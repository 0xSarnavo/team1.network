        ...
        <div className="space-y-2">
          {(users || []).map((u) => (
            <Link key={u.id} href={`/admin/users/${u.id}`}>
              <div className="flex items-center gap-4 rounded-lg border border-zinc-200 bg-white hover:border-zinc-300 dark:border-zinc-800 dark:bg-zinc-900/30 dark:hover:border-zinc-700 transition-colors cursor-pointer p-4">
                <Avatar src={u.avatarUrl} alt={u.displayName} size="sm" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="font-medium text-zinc-900 dark:text-zinc-100 truncate">{u.displayName}</p>
                ...
                <span className="text-xs text-zinc-500 dark:text-zinc-400">{new Date(u.createdAt).toLocaleDateString()}</span>
              </div>
            </Link>
          ))}
          ...
          {pagination && pagination.totalPages > 1 && (
            <div className="flex justify-center gap-2 pt-4">
              ...
              <span className="flex items-center text-sm text-zinc-600 dark:text-zinc-400">Page {page} of {pagination.totalPages}</span>
