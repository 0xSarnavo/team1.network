...
export default function AuditLogPage() {
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold text-zinc-900 dark:text-zinc-100">Audit Log</h1>
      <div className="mb-4 flex flex-wrap gap-3">
        <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search..." className="max-w-xs" />
        <select value={module} onChange={(e) => setModule(e.target.value)} className="rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-700 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300">
        ...
        <select value={severity} onChange={(e) => setSeverity(e.target.value)} className="rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-700 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300">
          <option value="">All Severity</option>
          <option value="normal">Normal</option><option value="sensitive">Sensitive</option><option value="critical">Critical</option>
        </select>
      </div>
      <DataTable columns={[
        { key: 'createdAt', header: 'Time', render: (r) => <span className="text-xs text-zinc-500 dark:text-zinc-400">{new Date(r.createdAt as string).toLocaleString()}</span> },
        ...
        { key: 'details', header: 'Details', render: (r) => <span className="text-xs text-zinc-500 dark:text-zinc-400 truncate max-w-[200px] block">{r.details as string || '—'}</span> },
