...
export default function RegionAdminEventsPage({ params }: { params: Promise<{ portalSlug: string }> }) {
  ...
  const [form, setForm] = useState({
    title: '', slug: '', description: '', markdown: '', type: 'meetup', status: 'draft',
  ...
  // Applications modal
  const [appModalEventId, setAppModalEventId] = useState<string | null>(null);
  const { data: applications, loading: appsLoading, refetch: refetchApps } = useApi<any[]>(
    appModalEventId ? `/api/portal/regions/${slug}/admin/events/${appModalEventId}/applications` : '',
    { immediate: !!appModalEventId }
  );
  const { mutate: patchApp } = useMutation<unknown>('patch');
  const [expandedAppId, setExpandedAppId] = useState<string | null>(null);

  const resetForm = () => {
    setForm({ title: '', slug: '', description: '', markdown: '', type: 'meetup', status: 'draft',
      visibility: 'member', location: '', isVirtual: false, virtualUrl: '',
      startDate: '', endDate: '', capacity: '' });
    setFormFields([]);
    setEditId(null);
              ...
              <CardContent className="flex items-center justify-between gap-4 py-3">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {e.formFields && e.formFields.length > 0 && (
                    <button onClick={() => setAppModalEventId(e.id)} className="rounded-lg p-1.5 text-zinc-400 hover:text-blue-500" title="View Applications">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-2xl rounded-2xl border border-zinc-200 bg-white p-6 shadow-2xl dark:border-zinc-800 dark:bg-zinc-950 max-h-[80vh] overflow-y-auto">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button onClick={() => { setAppModalEventId(null); setExpandedAppId(null); }} className="text-zinc-400 hover:text-zinc-600"><X className="h-5 w-5" /></button>
            </div>
            {appsLoading ? (
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {previewMode ? (
                  ...
                  <textarea value={form.markdown} onChange={(e) => setForm({ ...form, markdown: e.target.value })} rows={6} className="w-full rounded-lg border border-zinc-200 bg-card px-3 py-2 text-sm font-mono dark:border-zinc-800" placeholder="Write detailed event content in markdown..." />
                )}
              </div>
