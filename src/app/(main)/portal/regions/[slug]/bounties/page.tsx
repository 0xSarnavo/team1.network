...
export default function RegionBountiesPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      {/* Header */}
      <div className="mb-6">
        <Link href={`/portal/regions/${slug}/manage`} className="mb-3 inline-flex items-center text-sm text-zinc-500 hover:text-zinc-300">
          &larr; Back to Manage {regionInfo.region.name}
        </Link>
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-zinc-100">Bounty Management</h1>
            <p className="mt-1 text-zinc-500">Create and manage bounties for {regionInfo.region.name}</p>
          </div>
          <div className="flex gap-2">
            ...
            <Link href={`/portal/regions/${slug}/bounties/reviewers`}>
              <Button variant="outline" size="sm">Manage Reviewers</Button>
            </Link>
      ...
      {!bounties || bounties.length === 0 ? (
        <EmptyState
          ...
          description="Create your first bounty for this region."
